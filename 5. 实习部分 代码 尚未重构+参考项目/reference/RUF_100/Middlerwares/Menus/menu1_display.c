...
extern double g_flow_rate; //瞬时流量
extern double g_flow_info;    ///上电至今的流量

extern int flag_over;
extern int flag_reverse;
extern Parameters_t g_parameters;
// 历史流量数据
extern double empty;//占空比数值
// 全局配置
static DisplayMode current_mode = FLOW_RATE_MODE;
DisplayConfig mode_config[DISPLAY_MODE_COUNT];
//显示速度
extern int g_display_rate;

//显示平均下来的流速值，使得显示更平滑
#define AVERAGE_BUFFER_SIZE 20
static float flow_rate_buffer[AVERAGE_BUFFER_SIZE] = {0};
static int buffer_index = 0;
static int buffer_count = 0;
extern double filtered_flow_rate;
...
// 平均值计算函数
static float calculate_average(float* buffer, double new_value, int* buffer_index, int *buffer_count) 
{
    // 添加新值到缓冲区
...
{
    // 流速模式配置
    mode_config[FLOW_RATE_MODE] = (DisplayConfig){
        .title = "Flow Rate",
        .value = &g_flow_rate,
        .value_unit = "m/s",
        .secondary_value = &g_flow_info,
        .secondary_unit = "m3"
    };
}

// 显示模式固定为3个小数点前面的，一个小数点，两个小数点后面的
...
void display_flow_screen(void) 
{
    ...
    while(1) 
    {
        ...
        flow_rate_avg = calculate_average(flow_rate_buffer, *display_flow_rate, &buffer_index, &buffer_count);
        ...
        ui_display_set_value(value_str,flow_rate_unit_str,secondary_str,total_unit_str,
                                            doubleToString(g_GP22_data.empty_tube, 2),flag);
        // 按键处理
        ...
        // else if(KeyNum == KEY2_LONG_PRESS) { // 清零
        //     g_parameters.offset = filtered_flow_rate;
        //     SaveParameters(&g_parameters);
        // }
        
        vTaskDelay(pdMS_TO_TICKS((1000/g_parameters.sensitivity)));
    }
    // 理论上不会执行到这里，但为了安全
    free(display_flow_rate);
    free(display_flow_info);
...
// 主菜单处理
int menu1_start_handler(void* parameter) 
{
    flow_display_init();
    display_flow_screen();
    return 0;
