...
void lv_port_disp_init(void)
{
    ...
    /* 单缓冲区 (低RAM) */
    static lv_disp_draw_buf_t draw_buf_dsc_1;
    static lv_color_t buf_1[MY_DISP_HOR_RES * 4];                             /* 4 行屏幕的缓冲 */
    lv_disp_draw_buf_init(&draw_buf_dsc_1, buf_1, NULL, MY_DISP_HOR_RES * 4); /* 初始化显示缓冲区 */
...
static void disp_flush(lv_disp_drv_t *disp_drv, const lv_area_t *area, lv_color_t *color_p)
{
    ...
    /* Set window once and stream pixel data: far faster than LCD_DrawPoint per pixel. */
    LCD_Address_Set((u16)area->x1, (u16)area->y1, (u16)area->x2, (u16)area->y2);
    {
        const u16 w = (u16)(area->x2 - area->x1 + 1);
        const u16 h = (u16)(area->y2 - area->y1 + 1);
        const u32 px_cnt = (u32)w * (u32)h;

        for (u32 i = 0; i < px_cnt; i++)
        {
