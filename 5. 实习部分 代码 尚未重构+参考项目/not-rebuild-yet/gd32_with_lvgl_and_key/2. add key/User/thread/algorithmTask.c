...
{
    /* ---------- 累计流量 ---------- */
    g_algo.q_total_m3 = 0.0;

    /* ---------- SQ窗口 ---------- */
    memset(g_algo.bad_flags, 0, sizeof(g_algo.bad_flags));
    g_algo.sq_idx = 0;
    g_algo.sq_count = 0;
    g_algo.sq_bad_count = 0;

    /* ---------- 流速滑动窗口 ---------- */
    memset(g_algo.window_buf, 0, sizeof(g_algo.window_buf));
    g_algo.window_idx = 0;
    g_algo.step_cnt = 0;
    g_algo.window_full = false;

    /* ---------- 零漂补偿 ---------- */
    g_algo.zero_offset = 0.0f;
    g_algo.zero_stable = 0;
    ...
    xQueueOverwrite(xQueue_AlgoOut, &out);
}
...
void zero_drift_learn_start(void)
{
    ...
    algo_state_t *s = &g_algo;
    // 低通学习：offset 缓慢追踪 v（零流时 v≈offset）
    s->zero_offset = (1.0f - ALPHA) * s->zero_offset + ALPHA * inst_v;
...
static bool handle_group(algo_state_t *s, algo_out_t *out, double t1_ns, double t2_ns, double dt_ns)
{
    // 判坏：t1/t2 超限 或 dt 超限
...
void vAlgorithm_task(void *pvParameter)
{
    ...
    while (1)
    {
        ...
        if (xQueueReceive(xQueue_Rx_Index_Buf, &raw, portMAX_DELAY) == pdTRUE)
        {
            ...
            // 3) 处理一组数据
            if (handle_group(&g_algo, &out, t1, t2, dt))
            {
                printf("send data\r\n");
